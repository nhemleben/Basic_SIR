...
#Fourth draft of SIR Balls Model
#full motion and restrict motion clauses
...
frac_low_trigger = 5/100
frac_up_trigger = 10/100
...
colors=["white","white","white","white","white","white","white","white","white","white","white","white","white","white","white","white","yellow","grey"]
#90 healthy, 5% recover, 5% infected
...
while Nums[1] > 0:  #go while there are active infections
    ...
    #Once conditions are meet people begin being traped again
    if not freed and Nums[1] < frac_low_trigger * num_balls:
        print("Freed")
        for i in range(num_balls):
            if Imb[i]: #if previously not moving, change speeds to moving
                ...
                ball.dy=random.randint(-S,S)
                ball.dx=random.randint(-S,S)
        Imb = [False]*num_balls #Now everyone moves
        free_move = False
        freed  = True #now longer initate loop
        freed_times.append( len(Num_S) ) 
    elif freed and Nums[1] > frac_up_trigger * num_balls:
        print("Trapped")
        Imb = IMB
        for i in range(num_balls):
            if Imb[i]: #if previously moving, change speeds to not moving
                ball = balls[i]
                ball.dy= 0
                ball.dx= 0
        free_move = True
        freed  = False
        trapped_times.append( len(Num_S) ) 
        ...
        for j in range(i+1, num_balls):
            ...
            if dist <= 300*Rad:
                ...
                if Imb[i]:
                    ...
                    ball.dx *= -1
                    ball.dy *= -1
                else:  #since the masses are equal we just exchange velocities
...
for freed_time in freed_times:
    plt.axvline(freed_time,  label='Freedom Time', color = 'r')
for trapps in trapped_times:
    plt.axvline(trapps,  label='Trap Time', color = 'green')
